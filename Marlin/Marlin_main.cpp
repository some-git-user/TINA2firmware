		  ...
		  if (inp0.State() == PS_START)
		  {
			  ...
			  {	
				  if (serial_char == '\n' || serial_char == '\r')
				  {
					  ...
					  serial_line_buffer0[serial_count0] = 0;             
					  serial_count0 = 0;                                
				  ...
				  {   // Handle escapes
					  if ((c0 = MYSERIAL0.read0()) >= 0 && !serial_comment_mode0) 
					  {
...
inline void gcode_M17() {
   ...
   */
  void resume_print(const float &slow_load_length=0, const float &fast_load_length=0, const float &purge_length=ADVANCED_PAUSE_PURGE_LENGTH, const int8_t max_beep_count=0, const bool skipmenu = false) {
    ...
    if (nozzle_timed_out || thermalManager.hotEnoughToExtrude(active_extruder)) {
      ...
      load_filament(slow_load_length, fast_load_length, purge_length, max_beep_count, !skipmenu, nozzle_timed_out);
    }
...
inline void gcode_M42() {
  ...
  inline void gcode_M73() {
    if (!IS_SD_PRINTING && parser.seen('P') && wt_onlineprinting == SPARK_PRINTING)		
	{
...
inline void gcode_M502() {
  ...
  inline void gcode_M701() {
    ...
    load_filament(slow_load_length, fast_load_length, M701_PURGE_LENGTH, FILAMENT_CHANGE_ALERT_BEEPS,
...
void setup() {
  ...
  if (wt_get_hostmode() == HOST_WIIBUILDER)
	MYSERIAL0.begin(HOST_BAUDRATE, WIFI_BAUDRATE);
  else
	MYSERIAL0.begin(SPARK_BAUDRATE, WIFI_BAUDRATE);
  SERIAL_PROTOCOLLNPGM("start");
  SERIAL_PROTOCOLLNPGM("WTSTART");
  SERIAL_ECHO_START();
  ...
  servo_init();             // Initialize all servos, stow servo probe
	...
	strcpy_P(ipAddress, PSTR(MSG_WIFINOTCONNECTED));		
	strcpy_P(esp32_name, PSTR(MSG_UNKNOWN));
	strcpy_P(esp32_serial, PSTR(MSG_UNKNOWN));
	strcpy_P(esp32_version, PSTR(MSG_UNKNOWN));
	strcpy_P(esp32_hardware, PSTR(MSG_UNKNOWN));
	strcpy_P(tb_state, PSTR(MSG_UNKNOWN));
	...
	inp0.Reset();
	inp3.Reset();
  thermalManager.auto_report_temp_interval = 5;
}
